...
export async function DELETE(_req: NextRequest, { params }: RouteParams) {
  ...
  const { error: deleteRowError } = await supabase.from("task_photos").delete().eq("id", photoId);
  if (deleteRowError) {
    return NextResponse.json({ error: deleteRowError.message }, { status: 500 });
  }

  await supabase.storage.from(TASK_PHOTOS_BUCKET).remove([photo.storage_path]);
