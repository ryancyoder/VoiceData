...
export default function CalendarClient({
  ...
  // matched-or-new property plus the event.
  const [importSeed, setImportSeed] = useState<{ text: string; start?: string; end?: string } | null>(null);
  const makeAppointmentFromOutlook = (ev: OutlookEvt) => {
    const text = [
      ev.title && ev.title !== "(busy)" ? ev.title : "",
      ev.location ? `Location: ${ev.location}` : "",
      ev.description ?? "",
    ]
      .filter(Boolean)
      .join("\n");
    setImportSeed({ text, start: ev.start, end: ev.end });
  };
  ...
  async function handleRevertPhoto(photo: GeoPhoto) {
    // No native confirm() — it's unreliable in an iOS standalone PWA (returns
    // false without showing a dialog), which silently blocked revert. The
    // original is preserved server-side, so the annotation can be redone.
